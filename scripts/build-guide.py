...
def guide_anchors(lang):
    suffix = "" if lang == "en" else f".{lang}"
    source = Path("docs", f"enterprise-adoption{suffix}.md").read_text()
    md = markdown.Markdown(extensions=["toc"])
    md.convert(source.split("\n", 1)[1])

    def flatten(items):
        return [anchor for item in items
                for anchor in [item["id"], *flatten(item["children"])]]

    return flatten(md.toc_tokens)
...
def render(lang, copy):
    ...
    for other in LANGUAGE_ORDER:
        ...
        anchors = html.escape(json.dumps(guide_anchors(other)), quote=True)
        switcher += f'<a href="{guide_path(other)}" data-section-anchors="{anchors}" class="paper-btn language-link{active}" lang="{other}" hreflang="{other}"{current}>{other.upper()}</a>'
    switcher += "</div>"
